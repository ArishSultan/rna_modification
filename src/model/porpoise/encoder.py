...
def _encode_common(sequence: str, pse_knc_info=None):
    x1 = list(itemgetter(
...
def encode_fixed(sequence: str, sizes: tuple[int, int], matrices: tuple[list, list], pse_knc_info=None):
    if pse_knc_info is None:
        pse_knc_info = pse_knc.get_info()

    x0 = list(itemgetter(
        10, 12, 8, 9, 17, 7, 6, 15, 14, 11, 2, 13, 5, 16, 18, 4, 1, 3, 0
    )(pstnpss_new.encode(sequence, None, sizes, matrices)))

    x1, x2 = _encode_common(sequence, pse_knc_info)
    return x0 + x0 + x1 + x2
...
class EncoderFixed(BaseEstimator, TransformerMixin):
    ...
    def fit_transform(self, x: DataFrame, **kwargs) -> DataFrame:
        self._pstnpss_encoder.fit(x, kwargs['y'])
        return self.transform(x)

    def transform(self, x: DataFrame) -> DataFrame:
        encoder_lambda = lambda seq: encode_fixed(
            seq,
            self._pstnpss_encoder.sizes,
            self._pstnpss_encoder.matrices,
            self.pse_knc_info
        )

        return encode_df(x, encoder_lambda, 'porpoise')
