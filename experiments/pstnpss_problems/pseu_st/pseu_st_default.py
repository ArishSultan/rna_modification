...
human_experiment = ExperimentFixed(pseu_st.Factory(), human_test_dataset, human_train_dataset, encoder, k=10, feature_selector=human_feature_selector, use_targets_test=True)
human_report = human_experiment.run()
write_reports(human_report, 'pstnpss_problem_pseu-st_dl_human', Modification.psi.value, Species.human.value)

mouse_experiment = ExperimentFixed(pseu_st.Factory(), mouse_test_dataset, mouse_train_dataset, encoder, k=10, feature_selector=mouse_feature_selector, use_targets_test=True)
mouse_report = mouse_experiment.run()
write_reports(mouse_report, 'pstnpss_problem_pseu-st_dl_mouse', Modification.psi.value, Species.mouse.value)

yeast_experiment = ExperimentFixed(pseu_st.Factory(), yeast_test_dataset, yeast_train_dataset, encoder, k=10, feature_selector=yeast_feature_selector, use_targets_test=True)
yeast_report = yeast_experiment.run()
write_reports(yeast_report, 'pstnpss_problem_pseu-st_dl_yeast', Modification.psi.value, Species.yeast.value)
